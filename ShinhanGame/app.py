...
def main():
    ...
    <button id="sidebarToggleBtn" onclick="toggleSidebar()">💕 옵션</button>
    
    <script>
    function toggleSidebar() {
        // parent 문서에서 사이드바 요소 찾기
        ...
        if (sidebar) {
            ...
            const isHidden = sidebar.getAttribute('aria-expanded') === 'false' ||
                           window.parent.getComputedStyle(sidebar).marginLeft.includes('-');
        ...
        // Streamlit 기본 토글 버튼 찾아서 클릭 시도
        const toggleBtns = parent.querySelectorAll('button[kind="headerNoPadding"], [data-testid="collapsedControl"] button, [data-testid="stSidebarCollapsedControl"] button');
    ...
    function updateButtonVisibility() {
        ...
        if (sidebar && btn) {
            ...
            const isVisible = rect.width > 50 && rect.right > 0;
            
            // 사이드바가 보이면 버튼 숨김, 안보이면 버튼 표시
