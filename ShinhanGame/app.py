...
<style>
    ...
    /* 사이드바 펼쳐졌을 때 버튼 숨김 */
    .sidebar-open .floating-option-btn {
        opacity: 0 !important;
...
def main():
    ...
    st.markdown("""
    <button class="floating-option-btn" id="floatingOptionBtn" onclick="toggleSidebar()">
        💕 옵션
    </button>
    
    <script>
    function toggleSidebar() {
        // Streamlit 사이드바 토글 버튼 찾기
        const collapseBtn = window.parent.document.querySelector('[data-testid="collapsedControl"] button');
        if (collapseBtn) {
            collapseBtn.click();
        } else {
            // 대체 방법: baseButton 클래스의 버튼 찾기
            const altBtn = window.parent.document.querySelector('[data-testid="stSidebar"] button[kind="header"]');
    ...
    function checkSidebarState() {
        ...
        if (sidebar && btn) {
            // aria-expanded 또는 transform으로 상태 확인
            const isCollapsed = sidebar.getAttribute('aria-expanded') === 'false' || 
                              sidebar.style.transform.includes('-100') ||
                              sidebar.classList.contains('collapsed') ||
                              window.getComputedStyle(sidebar).transform.includes('matrix');
            
            // 사이드바 너비로 확인 (0이면 숨겨진 상태)
            const sidebarWidth = sidebar.getBoundingClientRect().width;
            const isHidden = sidebarWidth < 50;
