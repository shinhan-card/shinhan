    ...
    try:
        ...
        for model_name in model_names:
            try:
                ...
                # 실제로 모델이 작동하는지 간단한 테스트
                test_response = model.generate_content("Hi", generation_config={"max_output_tokens": 5})
                if test_response:
                    break
        ...
        if model is None:
            raise Exception(f"Gemini 모델 로드 실패: {last_error}")
...
{{"evaluation":"EXCELLENT/GOOD/AVERAGE/POOR","persuasion_gain":숫자,"customer_response":"캐릭터 말투로 자연스럽게 (이모지 1개)","feedback_positive":"구체적으로","feedback_improve":"개선점 + 예시","tip":"효과적인 멘트"}}'''
        ...
        if not response:
            ...
            return {
                ...
                "customer_response": "네, 계속 설명해주세요. 🤔",
                "feedback_positive": "",
                "feedback_improve": "상품의 구체적인 혜택과 수치를 언급해보세요.",
                "tip": "",
                "api_fallback": True
            }
        
        # Safety 필터로 차단된 경우 -> 로컬 평가로 폴백 (에러 대신)
        if response.candidates and len(response.candidates) > 0:
            ...
            if finish_reason != 1:  # 1=STOP(정상), 2=SAFETY, 3=RECITATION 등
                return {
                    "evaluation": local_eval,
                    "persuasion_gain": local_gain,
                    "customer_response": "음... 더 자세히 설명해주실 수 있어요? 🤔",
                    "feedback_positive": "",
                    "feedback_improve": "구체적인 수치와 혜택을 포함해서 설명해보세요.",
                    "tip": "",
                    "api_fallback": True
                }
        ...
        error_msg = str(e)
        if "404" in error_msg:
            print(f"[Gemini API Error] 404 - 모델을 찾을 수 없습니다. 모델명이 변경되었거나 API 키를 확인하세요.")
        elif "403" in error_msg:
            print(f"[Gemini API Error] 403 - API 키에 접근 권한이 없습니다.")
        elif "429" in error_msg:
            print(f"[Gemini API Error] 429 - API 호출 한도 초과. 잠시 후 다시 시도하세요.")
        else:
...
def init_session():
    defaults = {
        ...
        "api_key": "AIzaSyA9hix4O7y21I8Z27wVyO0U30140bCNytY",  # 기본 API 키
        "last_evaluation": "AVERAGE",
        "api_error": False,  # API 에러 플래그
        "company": "🏦 신한은행",
        "product": "SOL트래블 체크카드",
        "difficulty": "💀 Hard (매운맛)",
        "persona": "20대 사회초년생 김솔",
...
def start_game():
    ...
    first_messages = {
        "20대 사회초년생 김솔": f"아 저기요~ 친구가 {product} 좋다고 해서 왔는데요, 근데 솔직히 이런 거 잘 몰라서... 뭐가 좋은 건지 쉽게 설명해주실 수 있어요? 😅",
    ...
    with st.sidebar:
        ...
        with tab1:
            ...
            if st.session_state.game_mode == "ai":
                ...
                st.caption("✅ API Key 설정됨" if st.session_state.api_key else "[발급받기](https://aistudio.google.com/app/apikey)")
...
def render_game():
    ...
    if st.session_state.get("api_error"):
        st.toast("⚠️ AI 연결 문제 - 로컬 평가 사용 중")
...
def main():
    ...
    <button class="floating-option-btn" id="floatingOptionBtn">
        💕 옵션
    </button>
    
    <script>
    (function() {
        const btn = document.getElementById('floatingOptionBtn');
        if (!btn) return;
        
        // 버튼 클릭 시 사이드바 열기
        btn.addEventListener('click', function() {
            // parent document에서 Streamlit 토글 버튼 찾기
            const parentDoc = window.parent.document;
            
            // 방법 1: collapsedControl 내부 버튼
            let toggleBtn = parentDoc.querySelector('[data-testid="collapsedControl"] button');
            
            // 방법 2: baseButton-headerNoPadding 클래스
            if (!toggleBtn) {
                toggleBtn = parentDoc.querySelector('button[kind="headerNoPadding"]');
            }
            
            // 방법 3: 사이드바 닫기 버튼 (열려있을 때)
            if (!toggleBtn) {
                toggleBtn = parentDoc.querySelector('[data-testid="stSidebarCollapseButton"] button');
            }
            
            // 방법 4: 모든 header 버튼 중 첫번째
            if (!toggleBtn) {
                toggleBtn = parentDoc.querySelector('button[data-testid="baseButton-headerNoPadding"]');
            }
        ...
        // 사이드바 상태 감지 및 버튼 표시/숨김
        function checkSidebarState() {
            const parentDoc = window.parent.document;
            const sidebar = parentDoc.querySelector('[data-testid="stSidebar"]');
            
            if (sidebar) {
                // 사이드바 위치로 상태 확인 (transform이 -100%면 숨겨진 상태)
                const style = window.parent.getComputedStyle(sidebar);
                const transform = style.transform || style.webkitTransform;
                
                // 사이드바 rect로 확인
                const rect = sidebar.getBoundingClientRect();
                const isHidden = rect.right <= 0 || rect.width < 10;
                
                // collapsedControl이 보이는지 확인 (숨겨진 상태의 지표)
                const collapseControl = parentDoc.querySelector('[data-testid="collapsedControl"]');
                const hasCollapseBtn = collapseControl && collapseControl.offsetParent !== null;
                
                if (isHidden || hasCollapseBtn) {
